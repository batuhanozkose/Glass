...
pub fn toggle(
    ...
    if let Some((workspace, outline)) = workspace.zip(outline) {
        if workspace.read(cx).active_modal::<OutlineView>(cx).is_some() {
            // The same action can be re-dispatched while the picker is already open via
            // macOS key-equivalent routing. Ignore it instead of spawning a second modal.
            return;
        }
...
impl OutlineView {
    fn register(editor: &mut Editor, _: Option<&mut Window>, cx: &mut Context<Editor>) {
        if editor.mode().is_full() {
            ...
            editor
                .register_action(
                    move |action: &zed_actions::outline::ToggleOutline, window, cx| {
