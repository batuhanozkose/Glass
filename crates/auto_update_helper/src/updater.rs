...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("Glass.exe"), p("old\\Glass.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\Glass.exe"), p("old\\bin\\Glass.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\Glass.exe"), p("Glass.exe")),
        Job::move_file(p("install\\bin\\Glass.exe"), p("bin\\Glass.exe")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("Glass.exe"),
        app_dir.join("bin\\Glass.exe"),
        app_dir.join("bin\\glass"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("Glass.exe")).spawn();
    }
