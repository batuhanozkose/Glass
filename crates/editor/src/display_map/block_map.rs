...
impl From<BlockId> for ElementId {
    fn from(value: BlockId) -> Self {
        match value {
            ...
            BlockId::ExcerptBoundary(anchor) => {
                let opaque_id = anchor.opaque_id().unwrap();
                let stable_id = u64::from_le_bytes(opaque_id[..8].try_into().unwrap());
                ("ExcerptBoundary", stable_id).into()
            }
