        ...
        workspace.register_action(|workspace, _: &ToggleFocus, window, cx| {
            #[cfg(target_os = "macos")]
            {
                workspace.select_sidebar_section(
                    workspace::WorkspaceSidebarSection::Outline,
                    window,
                    cx,
                );
                let _ = workspace.focus_panel::<OutlinePanel>(window, cx);
            }
...
impl OutlinePanel {
            ...
            .flatten()
        {
            Some(serialization_key) => cx
                .background_spawn(async move {
                    GlobalKeyValueStore::global().read_kvp(&serialization_key)
                })
                .await
    ...
    fn update_search_matches(
        ...
        if buffer_search_matches.0.is_empty() && project_search_matches.is_empty() {
            if matches!(self.mode, ItemsDisplayMode::Search(_)) {
                self.mode = ItemsDisplayMode::Outline;
                update_cached_entries = true;
            }
        } else {
            let (kind, new_search_matches, new_search_query) = if buffer_search_matches.0.is_empty()
            {
                (
                    SearchKind::Project,
